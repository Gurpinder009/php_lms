...
if (isset($_POST["email"]) && $_POST["email"] !== "") {
    ...
    if (!isset($isRegistered["id"])) {
        ...
        if (sendMail($_POST["email"], $message)) {
            ...
            $data = ["staff_data" => $_POST, "hashedOtp" => $hashed_otp, "expire_time" => $expire_time];
            if (!isset($_SESSION)) {
                session_start();
            }
        ...
        redirect("staff/create", "Email is already registrated");
    }
} else {
    redirect("staff/create", "Necessary should be provided");
}
